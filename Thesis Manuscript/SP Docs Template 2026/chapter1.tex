...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item Evaluate and compare the performance of multiple machine learning models using metrics including Precision, Recall, F2-score, and Area Under the Precision-Recall Curve (AUPRC) Score for predicting diabetes risk among individuals in the Philippines via these methods
        \begin{enumerate}
            ...
            \item SMOTE
        \end{enumerate}
        \item Implement and compare the performance of multiple machine learning models, including:
        \begin{enumerate}
            \item Traditional ML: Naive Bayes, k-Nearest Neighbors, Random Forest, XGBoost, LightGBM, CatBoost, AdaBoost
            \item Deep Learning: TabNet with Bayesian Optimization
            \item Ensemble Methods: Voting Ensemble, Stacking Ensemble
        \end{enumerate}
    ...
    \begin{enumerate}
        ...
        \item Risk level classification (e.g., Low, Moderate, High)
        \item XAI explanations (SHAP/LIME) showing which factors contributed to the prediction
    \end{enumerate}
\end{enumerate}
\end{enumerate}
...
\begin{enumerate}
    ...
    \item It is assumed that the chosen machine learning models can effectively learn the patterns and relationships in the dataset for diabetes risk prediction. It is also assumed that the models will perform adequately after applying calibration techniques (such as Platt scaling and Isotonic Regression).
